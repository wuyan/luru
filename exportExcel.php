...
//获取岗位下的胜任力与子胜任力
$sql_competency    = "select * from position_competency where position_id='$_GET[id]'  order by cixun";
...
while ($row_competency = mysql_fetch_array($result_competency)) {
     ...
     array_push($rowArray, $row_competency_name[competency_name]);
    
    //向数组添加子胜任力
    $sql    = "select * from position_sub_competency where position_id='$_GET[id]' and competency_id=$row_competency[competency_id] order by cixun";
    $result = mysql_query($sql);
    while ($row = mysql_fetch_array($result)) {
        ...
        array_push($rowArray, $row_name[sub_competency_name]);
    }
