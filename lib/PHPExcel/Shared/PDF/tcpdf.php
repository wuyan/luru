...
// Esteban JoÎl MarÌn for OpenType font conversion.
...
class TCPDF {
	 ...
	 * v_ŒúÖ8gñ‚LYÈlq‚¸ò‚ÔÏﬂÍﬂïîû‘5ifRS≤ì\ñº©,%)ejJcJsJ©95=µojfÍ §Ç‘i¸3'u~*≥&µR¸H‡ôN§π˚EO˜-qΩªQ@ D qﬂxIÆ˚Ñ‰ŒÉ›ı"‚~#¢n£»s˜àõ›Éb®['ÜπÁ≈˜K1“˝B‹„÷ãËÌÆc‹7E°˚GQ‚~$ûq◊ä–˘Ù-B˙O§≈HØ∫üZß‹˘÷9wù¥‹?ãN"√≠ÉS@˝;PwµZ<Ö∂HO#ÕDöÖ4»/@ÓE§˘Hê∫ßçÀ‹¯NÜ{X¸‘çâæÓh;	á—Û0zFœFÙ:â'·'Óá¢è[É—ûÊ&`n7∏€Ä˚æx˜ûBöÅÙ4“L§YH›EG‹MãNËëÅqˆqwÁ<Ï≥6Ÿ&Ú]W‹ÊÇ]∂àª—^‡ûÅ∂7°ÌMh{⁄ﬁÑ∂7°ÌM8/^Aø
ZÜr9Ïµ:÷¢Ôz∑AT∫ó„:Hı‡˘xÓœ=‡∏ZÎ°µZÎ°µZÎ°µ£lÄ«e`.f"ÕBöç÷t∞˛3˝fz3oŸ∞A,6ò„√`€§gÓE‹_ÄTÅ{ÀêËõ	◊‚˛z§çnlΩS\≠ﬂ4f`º=›”–˙¥~≠&¥Æáˇ≠D∫«∫ì›∞–}∞ØØØi≈ÊÖS}C‡“Ë≥ˇ±Œ~˙úi´= ”í1C¶·Dó*Ä∞ÜÏ=ãz]ärrçÔ‘h®ÔË ú¶Kr»^Èdl]Ω†ëˆ€ÈWr˙EùñÙ›rπJFÈ•r}ûofpˆ°ô\¿≠åën8Ω9¬Ò√Ô≥÷¶Ω[;≥™˝Í,≥aÉ“’øs˝Îﬂ‹∞˛ÌZÚo)∞ñû[Ïzªﬂ#ªÜÊÉuE=B¡‘doU4;'{1Ê⁄Ó
,Ç$5ñS÷«ë¡å =Ÿa8;ÆÙ2ï…+“ﬂh⁄QX*”j‘Ûræ(‘ÿ¢QÆﬁ\Q„ÃîX°h‹0Ÿ†C[™IÙΩé¨óÂ~ÈúL[Km\!ˆhV!∫≈WX˜U†˛—êê3”ï≠iºøÎTÖº\¿8çƒÎXﬂ“ v‘>(H,Ç¥zÕÜ§D7ƒ∏ˆ=›nâ™¢RŒ;FÑâ©¡
˚=∞íîh°$≤2Zî¥ƒCœÛ∏=axzò≤ø∏“â6ÛbÌuÎbπÊÛê1˙Oû–"2∑˚Øõ1Ñõ˛ëlIt—Q‡ƒçq„.Ïè!ô~∑=CUÃå!~·…°‰⁄|4«êJ˛
Nı∏≈Ω›1è°cV†c∂ÛHÎ⁄ç øÓ4¢ÎgÌX*~Œ@param string $ht string to print as title on document header
	...
	public function Header() {
		...
		if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
			$this->Image(K_PATH_IMAGES.$headerdata['logo'], (0, 0, 0)));
	...
	/**
	 * Append a cubic BÈzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x2, y2) as the BÈzier control points.
	...
	/**
	 * Append a cubic BÈzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using the current point and (x2, y2) as the BÈzier control points.
	 * The new current point shall be (x3, y3).
	 * @param float $x2 Abscissa of control point 2.
	 * @param float $y2 Ordinate of control point 2.
	 * @param float $x3 Abscissa of end point.
	...
	/**
	 * Append a cubic BÈzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x3, y3) as the BÈzier control points.
	 * The new current point shall be (x3, y3).
	 * @param float $x1 Abscissa of control point 1.
	 * @param float $y1 Ordinate of control point 1.
	 * @param float $x2 Abscissa of control point 2.
	...
	protected function extractCSSproperties($cssdata) {
		...
		$cssdata = preg_replace('/@media[\s]+([^\{]*)\{/i', '@media \\1ß', $cssdata);
		$cssdata = preg_replace('/\}\}/si', '}ß', $cssdata);
		// trim string
		$cssdata = trim($cssdata);
		// find media blocks (all, braille, embossed, handheld, print, projection, screen, speech, tty, tv)
		$cssblocks = array();
		$matches = array();
		if (preg_match_all('/@media[\s]+([^\ß]*)ß([^ß]*)ß/i', $cssdata, $matches) > 0) {
			foreach ($matches[1] as $key => $type) {
				$cssblocks[$type] = $matches[2][$key];
			}
			// remove media blocks
			$cssdata = preg_replace('/@media[\s]+([^\ß]*)ß([^ß]*)ß/i', '', $cssdata);
		}
	...
	protected function SVGPath($d, $style='') {
		...
		foreach ($paths as $key => $val) {
			...
			switch (strtoupper($cmd)) {
				...
				case 'Q': { // quadratic BÈzier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
				...
				case 'T': { // shorthand/smooth quadratic BÈzier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if (($ck % 2) != 0) {
