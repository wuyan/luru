...
<head>
	...
	<?php 
			...
			$sql_sub_n = "select * from report_worth where position_id=$_GET[position_id] and page_id=$_GET[page_id]";
...
<body class='default' style="background-color:#FFF;width: 300px;">
    <form class="form" id="form"  method="post" action="relatetoworth.php" style="font-size: 13px; font-family: Verdana;">
        <div style='float: left;'>
            <div name="tree" id='jqxTree' style='float: left; margin-left: 20px;'>
				...
				<h2><?php echo "第".$_GET[page_id]."胜任力组";?></h2>
                    ...
                    <?php
					$sql = "select * from position_competency where competency_id =7 and position_id=$_GET[position_id] ";
					$result =mysql_query($sql);
					while ($row=mysql_fetch_array($result)){

						$sql_sub_n = "select * from position_sub_competency where competency_id=$row[competency_id] and position_id=$_GET[position_id]";
						$result_sub_n =mysql_query($sql_sub_n);
						while ($row_sub_n=mysql_fetch_array($result_sub_n)){
							...
							/*
							if($_GET[page_id]>1){
								$sql_sub_all = "select * from report_worth where position_id=$_GET[position_id] ";
