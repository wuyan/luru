...
if (isset($_GET['update'])) {
    ...
    //获取自我管理分数
    $sql_ziwo      = "select * from score_competency where competency_id=6 and user_id='" . mysql_real_escape_string($_GET['用户ID']) . "'";
    ...
    if ($total_averge2 >= 5 or ($total_averge2 >= 4.5 and $total_averge2 < 5 and $ziwo >= 5)) {
        $update_query = mysql_query("update `user_basic` set  `user_info_value`='通过'  WHERE `user_id`='" . mysql_real_escape_string($_GET['用户ID']) . "' and `user_info_id`='5'");
    } elseif ($total_averge2 < 4.5 or ($total_averge2 >= 4.5 and $total_averge2 < 5 and $ziwo < 5)) {
        $update_query = mysql_query("update `user_basic` set  `user_info_value`='待提升'  WHERE `user_id`='" . mysql_real_escape_string($_GET['用户ID']) . "' and `user_info_id`='5'");
    }
    ...
    //通过岗位ID得到项目ID
    $sql_position    = "select * from position_list where subcompetencyidall='$_GET[modesum]'";
    $result_position = mysql_query($sql_position);
	$j=0;
    while ($row_position = mysql_fetch_array($result_position)) {
        ...
        $i           = $j;
        $mysql_init  = $mysql_init . "user_profile.user_id from user_basic,user_profile where user_profile.position_id=$row_position[position_id] and user_profile.user_id=user_basic.user_id and user_profile.shenhe=0 group by user_id ";
        ...
        $i           = $j;
        $mysql_init2 = $mysql_init2 . "user_profile.user_id from score_competency,user_profile where user_profile.position_id=$row_position[position_id] and user_profile.user_id=score_competency.user_id and user_profile.shenhe=0 group by user_id ";
        ...
        $mysql_init1       = "select ";
        $sql_competency    = "select * from position_sub_competency where position_id='$row_position[position_id]'  order by cixun";
        $result_competency = mysql_query($sql_competency);
        while ($row_competency = mysql_fetch_array($result_competency)) {
        ...
        $i           = $j;
        $mysql_init1 = $mysql_init1 . "user_profile.user_id from score,user_profile where user_profile.position_id=$row_position[position_id] and user_profile.user_id=score.user_id and user_profile.shenhe=0  group by user_id ";
        ...
        while ($row_name = mysql_fetch_array($result_name)) {
            ...
            while ($row_competency = mysql_fetch_array($result_competency)) {
                //向数组添加子胜任力
                $sql_subcompetency    = "select * from position_sub_competency where position_id='$row_position[position_id]' and competency_id=$row_competency[competency_id] order by cixun";
                $result_subcompetency = mysql_query($sql_subcompetency);
                while ($row_subcompetency = mysql_fetch_array($result_subcompetency)) {
