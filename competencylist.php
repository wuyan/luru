	...
	<?php 
			echo " 
				<script type='text/javascript'>
					...
					$sql_sub_n = "select * from position_competency where position_id=$_GET[id]";
...
<body class='default' style="background-color:#FFF;">
    <form class="form" id="form" target="form-iframe" method="post" action="positioncompetency.php" style="font-size: 13px; font-family: Verdana; width: 400px;">
                ...
                <ul>
                    ...
                    <?php
						...
						while ($row_sub_n=mysql_fetch_array($result_sub_n)){
									...
									$sql_sub_c = "select * from position_competency where competency_id='$row_sub_n[competency_id]' and position_id='$_GET[id]'";
									$result_sub_c =mysql_query($sql_sub_c);
									$row_sub_c=mysql_fetch_array($result_sub_c);
									echo "</br>权重：<input type='text' name='weight".$row_sub_n[competency_id]."' value='$row_sub_c[weight]'/>";
