...
?>
    <div class="container">
        ...
        <div class="left">
            <div class="xuanzeBox">
                <h5>当前位置： 网站首页 > 录入参试者分组</h5>
            </div>
            <div class="content">
                <div class="pubTitle">
                    <h1><font>录入参试者分组</font></h1>
                </div>
				<form action="uploadceping.php" method="post" enctype="multipart/form-data">
                <div class="pubCon">
                        <h2>STEP1：选择所选客户</h2>
                        ...
                        <div class="fdDiv">
                            <span>
								<select size="1" class="selectdiv" id='position_name' name="position_id" onchange="xajax_menu_ceping(document.getElementById('position_name').value); return false;">
							<option  ></option>
							<?php
							...
							<span id='exporttemplate'>
								<font><a href="exportExcel.php?id=<?php
									$sql_project = "select * from project_list where group_id=$row_group[group_id]";
									$result_project =mysql_query($sql_project);
									$row_project=mysql_fetch_array($result_project);
									$sql_position = "select * from position_list where project_id=$row_project[project_id]";
									...
									echo $row_position[position_id];
								?>"class="xblue">下载岗位模板数据</a></font>
							</span>
                            <div class="clear"></div>
                        </div>

                        <div class="divupdata">
                            <div class="fileupdata">选择文件</div>
                            <input name="upfile" type="file" value="" class="inputstyle"  accept=".xls"/>

                            <input  type="submit" value="上传" class="uploadbutton" />
