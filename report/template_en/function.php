...
function competency_ld($competency, $score){
	...
	if ( $score>0 and $score <=1){
		...
		return $result['summary1_en'];
	};
	if ( $score>1 and $score <=2){
	
		return $result['summary2_en'];
	};
	if ( $score>2 and $score <=3){
	
		return $result['summary3_en'];
	};
	if ( $score>3 and $score <=4){
	
		return $result['summary4_en'];
	};
	if ( $score>4 and $score <=5){
	
		return $result['summary5_en'];
	};
	if ( $score>5 and $score <=6){
	
		return $result['summary6_en'];
	};
	if ( $score>6 and $score <=7){
	
		return $result['summary7_en'];
	};
	if ( $score>7 and $score <=8){
	
		return $result['summary8_en'];
	};
	if ( $score>8 and $score <=9){
	
		return $result['summary9_en'];
	};
	if ( $score>9 and $score <=10){
	
		return $result['summary10_en'];
	};
...
function competency_zh($competency, $score){
	...
	$sql= mysql_query("select * from report_sub_competency where sub_competency_id='$competency' and report_id=$_GET[report_id] ");
	$result = mysql_fetch_array($sql);
