...
<div class="central">
	...
	<div class="main">
       ...
       岗位:
			...
			<div class="shouye_v1">
        &nbsp;<?php 
		$sql_name = "select * from  position_list where position_id =$row_position[position_id]";
		$result_name =mysql_query($sql_name);
		$row_name=mysql_fetch_array($result_name);
		echo $row_name[position_name];
	   ?>
			</div>
		</div>
		<div class="shouye_all">
       ...
       公司:
			...
			<div class="shouye_v1">
        &nbsp;<?php 
		$sql_name = "select * from  user_basic where user_id =$row[user_id] and user_info_id=7";
		$result_name =mysql_query($sql_name);
		$row_name=mysql_fetch_array($result_name);
		echo $row_name[user_info_value];
	   ?>
...
$sql_content = "select * from report_worth where position_id=$row_position[position_id]";
$result_content =mysql_query($sql_content);
while($row_content=mysql_fetch_array($result_content))
{
?>
...
if (AC_FL_RunContent == 0 || DetectFlashVer == 0) {
	...
	if(hasRightVersion) { 
		AC_FL_RunContent(
			...
			'movie', 'charts',
			'src', 'charts',
			'FlashVars', 'library_path=charts_library&xml_source=vl_s.php?pageid=<?php echo $row_content[page_id]?>&positionid=<?php echo $row_position[position_id]?>', 
			'id', 'my_chart',
			'name', 'my_chart',
			'menu', 'true',
			'allowFullScreen', 'true',
...
// -->
      ...
      标杆得分
		...
		<div style="width:750px;border:0px solid black;text-align:center;font-size:13pt;margin-top:80px;">
       <?php echo $row_content[title]?>承诺等级
		</div>
		<table style="width:750px;border:0px solid black;font-size:10pt;text-align:center;margin-top:10px;" cellpadding="0" cellspacing="2">
		<tbody>
		<tr style="height:50px;border:0px solid black;background-color:#4F81BD;color:ffffff;">
