...
if ($_GET['delete'] == 'true') {
    ...
    for ($index = 0; $index < count($tree); $index++) {
        ...
        $result = mysql_query("update `user_profile` set  `shenhe`='2' where user_id=$tree[$index]");
    ...
    while ($row_position = mysql_fetch_array($result_position)) {
        ...
        $i           = $j;
        $mysql_init  = $mysql_init . "user_profile.user_id ,user_profile.shenhe from user_basic,user_profile where user_profile.position_id=$row_position[position_id] and user_profile.user_id=user_basic.user_id and user_profile.shenhe>0 group by user_id ";
        ...
        while ($row_name = mysql_fetch_array($result_name)) {
            ...
            while ($row_profile = mysql_fetch_array($result_profile)) {
                ...
                if ($row_name['shenhe'] == 1) {
                    
                    $customers[$i]['status'] = '已审核数据';
                } else {
                    $customers[$i]['status'] = '归档数据';
                }
