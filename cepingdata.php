...
if (isset($_GET['update'])) {
    ...
    for ($index = 0; $index < count($tree); $index++) {
		//将数据同步到未审核数据中
		$user_profile="select * from user_profile_ceping where user_id='$tree[$index]'";
		$result_user_profile = mysql_query($user_profile);
		$row_user_profile = mysql_fetch_array($result_user_profile);
		mysql_query("INSERT INTO `user_profile` (`name`,`position_id`,`ceping_person`,`ceping_date`,`shenhe`) VALUES('$row_user_profile[name]','$row_user_profile[position_id]','$row_user_profile[ceping_person]','$row_user_profile[ceping_date]',0)");

		$max="select max(user_id) as max from `user_profile`";
		$result_max = mysql_query($max);
		$row_max = mysql_fetch_array($result_max);

		$user_basic="select * from user_basic_ceping where user_id='$tree[$index]'";
		$result_user_basic = mysql_query($user_basic);
		while($row_user_basic=mysql_fetch_array($result_user_basic)){
		 mysql_query("INSERT INTO `user_basic` (`user_id`,`user_info_id`,`user_info_value`) VALUES('$row_max[max]','$row_user_basic[user_info_id]','$row_user_basic[user_info_value]')");
		}

		$user_score="select * from score_ceping where user_id='$tree[$index]'";
		$result_score = mysql_query($user_score);
		while($row_score=mysql_fetch_array($result_score)){
			mysql_query("INSERT INTO `score` (`user_id`,`position_id`,`sub_competency_id`,`score`) VALUES('$row_max[max]','$row_score[position_id]','$row_score[sub_competency_id]','$row_score[score]')");
		}

		$user_score="select * from score_competency_ceping where user_id='$tree[$index]'";
		$result_score = mysql_query($user_score);
		while($row_score=mysql_fetch_array($result_score)){
			mysql_query("INSERT INTO `score_competency` (`user_id`,`position_id`,`sub_competency_id`,`score`) VALUES('$row_max[max]','$row_score[position_id]','$row_score[sub_competency_id]','$row_score[score]')");
		}


		$result = mysql_query("update `user_profile_ceping` set  `shenhe`='1' where user_id=$tree[$index]");
    }
	$tishi="测评分数审核成功";
	...
	user_profile_ceping.user_id=user_basic_ceping.user_id and user_profile_ceping.shenhe=0  and 
    ...
    $i = 0;
    $mysql_init2 = $mysql_init2 . "user_profile_ceping.user_id from score_competency_ceping,user_profile_ceping where user_profile_ceping.position_id=$_GET[position_id] and user_profile_ceping.user_id=score_competency_ceping.user_id and user_profile_ceping.shenhe=0 and 
	user_profile_ceping.ceping_person='$ceping_person' group by user_id ";
    ...
    $i = 0;
    $mysql_init1 = $mysql_init1 . "user_profile_ceping.user_id from score_ceping,user_profile_ceping where user_profile_ceping.position_id=$_GET[position_id] and user_profile_ceping.user_id=score_ceping.user_id and user_profile_ceping.shenhe=0  and 
	user_profile_ceping.ceping_person='$ceping_person' group by user_id ";
